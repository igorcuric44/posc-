...
arr = new string[7];
	...
	try
		{
		con.Open();
		if(Convert.ToInt32(results[ii][2])!=0 && Convert.ToInt32(results[ii][6])==0){
		sql=@"select * from skladiste where id in
			(select id1 from sifrarnik where id="+(results[ii][0]).ToString()+");";
		...
		Console.WriteLine("++++"+mjeraxx*kolicina);
		
		cmd=con.CreateCommand();
		cmd.CommandText=@"update skladiste set kolicina="+kolicina+" where id="+
		...
		Console.WriteLine(Convert.ToInt32(results[ii][6]));
		}
