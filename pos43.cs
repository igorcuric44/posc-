...
private void InitalizeComponent()
{
	...
	suma = new Label();
    suma.Location =new Point((pnlContainer.Width-suma.Width)/2,(pnlContainer.Height-suma.Height)/2);;
...
private void btn1_click(object sender, EventArgs  e) {
	...
	Console.WriteLine(pnlContainer.ClientRectangle.Height);
	Console.WriteLine(pnlContainer.ClientRectangle.Width);
...
private void txtInput_KeyPress(object sender ,KeyPressEventArgs e){
	...
	if(keyChar==13 && p==1){
		...
		{
		con.Open();
		sql=@"select kolicina from skladiste where id in
			(select id1 from sifrarnik where id="+str2+");";
    	cmd=new SqliteCommand(con);
		cmd.CommandText=sql;
		float kolicina=Convert.ToSingle(cmd.ExecuteScalar());
		...
		itemx1.Text=(s*Convert.ToSingle(itemx.Text)).ToString();
		sumax=sumax+Convert.ToSingle(itemx.Text);
		suma.Text=sumax.ToString();
...
arr = new string[6];
		...
		con.Open();
		sql=@"select kolicina from skladiste where id in
			(select id1 from sifrarnik where id="+str1+");";
    	cmd=new SqliteCommand(con);
		cmd.CommandText=sql;
		float kolicina=Convert.ToSingle(cmd.ExecuteScalar());
    	Console.WriteLine(kolicina);
...
private void tipka1_click(object sender,EventArgs e) {
	...
	try
	{
		...
		id2,mjera2,cijena2,pdv2) values ("+sifra+",'"+usluga+"',"+id1+","+mjera1+","+cijena1+","+pdv1
		+",0,0.0,0.0,0.0);";  
			cmd.ExecuteNonQuery();
		}
		else
